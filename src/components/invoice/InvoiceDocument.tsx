    ...
    <div className="w-full mx-auto print:w-full print:max-w-none print:h-auto print:overflow-visible">
      ...
      >
        ...
        <div className="p-8 md:p-12 print:p-4 flex-grow print:flex-grow-0 flex flex-col">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 print:grid-cols-2 gap-8 print:gap-4 mb-8 print:mb-4">
            ...
            <div className="flex flex-col items-start">
              {logoUrl ? (
                ...
                <div className="h-20 flex items-center mb-4 print:mb-2 print:h-16">
              ...
              <div className="text-sm text-gray-600 space-y-1 print:text-xs print:leading-snug print:space-y-0.5">
                <p className="font-semibold text-gray-900 text-base print:text-sm mb-1 print:mb-0.5">{business?.name}</p>
                ...
                <div className="mt-4 pt-2 text-xs text-gray-500 space-y-0.5 border-t border-gray-100 print:mt-2 print:pt-2 print:border-gray-200">
                  {isAE ? (
                    <>
            ...
            <div className={`flex flex-col ${isRTL ? "items-start text-right" : "items-end text-right"}`}>
              ...
              <div className="mb-6 print:mb-4 w-full">
                <h1 className="text-4xl font-light tracking-tight mb-1 uppercase text-gray-900 print:text-3xl print:font-bold print:mb-1">
                  {invoice.type === "quote" ? labels.quote : invoice.type === "credit_note" ? labels.credit_note : labels.invoice}
                </h1>
                <p className="text-lg font-medium text-gray-500 mb-4 print:mb-2 print:text-base">#{invoice.invoiceNumber}</p>
              ...
              <div className={`w-full bg-gray-50/50 rounded-lg p-4 border border-gray-100 print:p-3 print:bg-transparent print:border-gray-200 text-left ${isRTL ? "text-right" : "text-left"}`}>
                <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2 print:mb-1">{labels.billTo}</h3>
                <h2 className="font-bold text-lg text-gray-900 mb-1 print:text-base print:mb-0.5">{invoice.customer?.name}</h2>
                {invoice.customer?.contactPerson && (
                  <p className="text-sm text-gray-600 mb-1 print:text-xs print:mb-0.5">Attn: {invoice.customer.contactPerson}</p>
                )}

                <div className="text-sm text-gray-600 space-y-1 print:text-xs print:leading-snug print:space-y-0.5">
                  <p className="whitespace-pre-line">{invoice.customer?.address}</p>
                  <div className="grid grid-cols-1 gap-y-0.5 mt-2 text-xs text-gray-500 border-t border-gray-200 pt-2 print:mt-1 print:pt-1 print:text-[10px]">
          ...
          {/* Items Table */}
          <div className="mb-8 mt-8 print:mb-4 print:mt-4">
            <table className="w-full text-sm print:text-xs">
              ...
              <tbody className="divide-y divide-gray-50">
                {items?.map((item, index) => (
                  <tr key={index}>
                    <td className={`py-3 print:py-1 ${isRTL ? "pr-2" : "pl-2"} text-gray-900 font-medium`}>{item.description}</td>
                    <td className="py-3 print:py-1 text-right text-gray-600">{item.quantity}</td>
                    <td className="py-3 print:py-1 text-right text-gray-600">
                      {item.unitPrice.toLocaleString("en-US", { minimumFractionDigits: 2 })}
                    </td>
                    {!isAE && <td className="py-3 print:py-1 text-right text-gray-600">{item.tvaRate}%</td>}
                    <td className={`py-3 print:py-1 ${isRTL ? "pl-2" : "pr-2"} text-right text-gray-900 font-medium`}>
          ...
          {/* Totals & Notes */}
          <div className="flex flex-col md:flex-row print:flex-row gap-8 print:gap-4 mb-8 print:mb-4 print:break-inside-avoid">
          ...
          {/* Amount in Words */}
          <div className={`mb-8 print:mb-4 print:break-inside-avoid flex flex-col ${isRTL ? "items-end text-right" : "items-start text-left"}`}>
          ...
          <div className="flex justify-end mb-8 print:mb-0 print:break-inside-avoid">
            <div className="w-[400px] print:w-[500px] text-center relative">
              <p className="text-sm font-semibold text-gray-900 mb-4 print:mb-2 print:text-xs">{labels.signature}</p>
              
              {/* Adjusted height for print */}
              <div className="h-48 print:h-48 w-full flex items-center justify-center relative">
                {/* Stamp Layer */}
                {stampUrl && (
                  <img 
                    src={stampUrl} 
                    alt="Stamp" 
                    className="absolute right-0 top-0 w-48 h-48 print:w-48 print:h-48 object-contain opacity-80 rotate-[-12deg] mix-blend-multiply" 
                  />
                )}
          ...
          {/* Footer */}
          <div className="mt-auto print:mt-4 pt-6 print:pt-2 border-t border-gray-100 text-center text-xs text-gray-400 print:text-[10px] print:break-inside-avoid">
